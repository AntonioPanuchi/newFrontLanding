...
- Личный кабинет пользователя с авторизацией через локальное хранилище
...
- **CI/CD:** GitHub Actions, PM2 _(workflow требует настройки)_
...
_Ниже приведён пример workflow для GitHub Actions. Создайте файл `.github/workflows/ci-cd.yml` в своём репозитории и адаптируйте его под свои нужды._
...
- `GET /api/xui/inbounds` — список инбаундов 3x-ui (id, remark, enable, port, protocol, up, down, total, expiryTime)
...
- `.github/workflows/ci.yml` _(пример конфигурации)_
- `deploy.sh`
...
> 🧠 _Документ является живым. Новые агенты и изменения добавляются по мере развития проекта._
...
_Google Analytics 4, Core Web Vitals — требуют отдельной настройки и интеграции._
