...
const MyDataPage: React.FC = () => {
  ...
  const handleAdd = () => {
    if (file) {
      addDocument({ id: Date.now(), title: file.name, type: file.type || 'file', date: new Date().toISOString() });
      setFile(null);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2">
        ...
        <button type="button" onClick={handleAdd} className="px-4 py-2 bg-blue-500 text-white rounded-md">
          Загрузить
        </button>
      </div>
      <ul className="space-y-2">
        ...
        {docs.length === 0 && <li className="text-sm text-gray-500">Нет данных</li>}
      </ul>
    </div>
  );
};
